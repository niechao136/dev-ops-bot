...
import { ChatConversation, ChatMessage, ChatReq } from '@/types/chat';
import { getToken } from '@/utils/cookie';


export async function getConversationList() {
  const res = await apiRequest(`/chat`);
  const data: DataResult<ChatConversation[]> = await res.json();
  return data;
}


export async function getChatHistory(conversation_id: string) {
  const res = await apiRequest(`chat/${conversation_id}`);
  const data: DataResult<ChatMessage[]> = await res.json();
  return data;
}
