...
from src.schemas.chat import ChatRequest, ChatResponse, ConversationItem, MessageItem
from src.utils.auth import get_current_user
from src.utils.chat import check_chat_access, get_chat_config, get_content
...
@chat_router.get(
    ...
    summary="获取会话列表",
    description="查询当前用户所有历史对话记录，并按最后更新时间倒序排列。"
)
async def get_all_conversations(
        user: Annotated[TokenDict, Depends(get_current_user)],
...
async def send_message(
    ...
    """
    agent, conversation_id, config = await get_chat_config(body, user, pool)
